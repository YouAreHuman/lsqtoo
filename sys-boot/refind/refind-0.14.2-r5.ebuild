...
PATCHES=(
	...
	"${FILESDIR}"/${P}-fix-gnu-efi-4.0.patch
)
...
src_compile() {
	...
	tc-export CC
	# bug #944562 - gnu-efi 4.0+ compatibility
	local -x EXTRACFLAGS="-DGNU_EFI_3_0_COMPAT -DGNU_EFI_USE_COMPAREGUID_ABI=0"
	if tc-is-gcc; then
		...
		EXTRACFLAGS+=" -D__DEFINED_wchar_t"
	fi
